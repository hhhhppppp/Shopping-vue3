...
const rules = {
  ...
  password: [
    ...
    {min: 6, max: 14, message: '密码为6-14个字符', triger: 'blur'}
  ],
  agree: [
    {
      // 自定义规则
...
const onLogin =  () => {
  ...
  formRef.value.validate( async (valid) => {
    ...
    if (valid) {
      ...
      router.replace({path: '/'})
      // 已解决！！
      // 购物车 逻辑 登陆后 需要重新添加一个商品才能看到未登录下选择的商品
      // const {skuId, count} = await findNewCartListAPI()
      // useCartStore().addCart({skuId, count})
    }
  })
}
</script>
